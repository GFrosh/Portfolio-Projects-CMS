...
export default function ProjectForm({ initial, onSubmit, onCancel, submitLabel }: ProjectFormProps) {
  ...
  const fetchRepos = async () => {
    ...
    setSelectedRepo('');
    try {
      const res = await fetch(`https://api.github.com/users/${encodeURIComponent(username)}/repos?per_page=100&sort=updated`, { headers: { Accept: 'application/vnd.github+json' } });
      if (!res.ok) {
  ...
  const fetchRepoDetailsAndFill = async (repoFullName: string) => {
    ...
    setGithubError('');
    try {
      const res = await fetch(`https://api.github.com/repos/${repoFullName}`, { headers: { Accept: 'application/vnd.github+json' } });
