...
export default function DashboardShell({ user, signOut }: DashboardShellProps) {
  ...
  const initials = displayName.split(/\s+/).map((s) => s.charAt(0).toUpperCase()).slice(0, 2).join('');
    ...
    <div className={styles.dashboardShell}>
      ...
      {modal?.type === 'create' && (
        <Modal title="New project" subtitle="Add a new entry to your portfolio." onClose={closeModal} size="lg">
          <ProjectForm submitLabel="Create project" onCancel={closeModal} onSubmit={(data) => { addProject(data); closeModal(); }} />
        </Modal>
      )}

      {modal?.type === 'edit' && (
        <Modal title="Edit project" subtitle={modal.project.title} onClose={closeModal} size="lg">
          <ProjectForm initial={{ ...modal.project }} submitLabel="Save changes" onCancel={closeModal} onSubmit={(data) => { updateProject(modal.project.id, data); closeModal(); }} />
        </Modal>
      )}
